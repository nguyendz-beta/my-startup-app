...
export default function CustomerOrderPage() {
  ...
  useEffect(() => {
    if (!branchId) return;
    Promise.all([
      axios.get(`/api/products/public?branchId=${branchId}`),
      ...
      .then(([prodRes, tableRes]) => {
        ...
        const cats = [
          'Tất cả',
          ...(Array.from(
            new Set(prods.map((p: Product) => p.category?.name).filter(Boolean)),
          ) as string[]),
        ];
        setCategories(cats);
        ...
        <div className="fixed inset-0 bg-black/50 z-30 flex items-end">
          <div className="bg-white w-full rounded-t-2xl p-6 max-h-[80vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowCart(false)} className="text-gray-400 text-xl">
                ✕
              </button>
            </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() =>
                        setCart((prev) =>
                          prev.map((i2) =>
                            i2.productId === item.productId && i2.variantId === item.variantId
                              ? { ...i2, quantity: i2.quantity + 1 }
                              : i2,
                          ),
                        )
                      }
