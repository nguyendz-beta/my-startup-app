      ...
      {lowStock.length > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-4">
          <p className="font-semibold text-red-700 mb-2">
            ⚠️ Sắp hết hàng ({lowStock.length} mặt hàng)
          </p>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                {['Nguyên liệu', 'Danh mục', 'Tồn kho', 'Đơn vị', 'Giá nhập', 'Trạng thái', ''].map(
                  (h) => (
              ...
              {filtered.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`font-bold ${item.quantity <= item.minQuantity ? 'text-red-500' : 'text-gray-800'}`}
                    >
                      {fmt(item.quantity)}
                    </span>
                    <span className="text-xs text-gray-400 ml-1">
                      / min {fmt(item.minQuantity)}
                    </span>
                  </td>
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  ...
                  <span className="font-bold text-orange-600">
                    {fmt(selected.quantity)} {selected.unit}
                  </span>
                </p>
              </div>
              <button
                ...
                {logs.length === 0 ? (
                  ...
                  logs.map((log) => (
                    <div key={log.id} className="flex items-center justify-between text-xs">
                      <span
                        className={`px-2 py-0.5 rounded-full font-medium ${TYPE_LABEL[log.type]?.color}`}
                      >
                        {TYPE_LABEL[log.type]?.label}
                      </span>
                      <span className="font-bold">
                        {log.type === 'OUT' ? '-' : '+'}
                        {fmt(log.quantity)} {selected.unit}
                      </span>
